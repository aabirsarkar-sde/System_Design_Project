...
import { buildAppContext } from "./bootstrap/appContext";
...
const requestStatusOrder: ServiceRequestStatus[] = ["PENDING", "DISPATCHED", "IN_PROGRESS", "SCHEDULED", "RESOLVED"];
...
function categoryLabel(category: ServiceRequestCategory): string {
  return category.replace("_", " ");
}

function iconForCategory(category: ServiceRequestCategory): string {
  switch (category) {
    case "MAINTENANCE":
      return "wrench";
    case "IT_SUPPORT":
...
function priorityWeight(priority: ServiceRequestPriority): number {
  switch (priority) {
    case "EMERGENCY":
      return 4;
    case "HIGH":
...
function validateCreateRequestPayload(payload: unknown): ServiceRequestInput & { userId: string } {
  ...
  const allowedCategories: ServiceRequestCategory[] = [
    "MAINTENANCE",
    "IT_SUPPORT",
    "HOUSEKEEPING",
    "SECURITY",
  ...
  const allowedPriorities: ServiceRequestPriority[] = ["LOW", "MEDIUM", "HIGH", "EMERGENCY"];
  ...
  if (typeof category !== "string" || !allowedCategories.includes(category as ServiceRequestCategory)) {
    throw new Error("category is invalid");
  }

  if (typeof priority !== "string" || !allowedPriorities.includes(priority as ServiceRequestPriority)) {
  ...
  return {
    ...
    building,
    room,
    priority: priority as ServiceRequestPriority,
  };
}
...
app.get("/api/header/me", (req: Request, res: Response) => {
...
app.get("/api/dashboard/resident", (req: Request, res: Response) => {
  const userId = ensureUserId(req.query.userId);
  const user = appContext.userRepository.findById(userId);
  if (!user) {
    res.status(404).json({ message: "User not found" });
...
app.get("/api/analytics", (_req: Request, res: Response) => {
  ...
  const categoryCount = requests.reduce<Record<string, number>>((accumulator, request) => {
  ...
  const topCategories = Object.entries(categoryCount)
    .map(([category, count]) => ({
      category: categoryLabel(category as ServiceRequestCategory),
...
app.get("/api/facilities", (_req: Request, res: Response) => {
  const details: Record<string, { location: string; capacity: string; hvacStatus: string; powerStatus?: string; status: string }> = {
...
app.get("/api/requests/board", (req: Request, res: Response) => {
  const userId = ensureUserId(req.query.userId);
  const requests = appContext.requestRepository.findByUserId(userId);
